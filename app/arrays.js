arraysAnswers = {
  ...
  remove: function remove(arr, item) {
    //WORKS WELL IN BROWSER, PLEASE CHECK ONCE
  ...
  duplicates: function duplicates(arr) {
    const arr2 = [];
    arr.forEach(n => {
      //IF FIRST AND LAST INDEX ARE NOT SAME(REPEATED) , INCLUDE IN RESULT (if not included)
      if (arr.indexOf(n) !== arr.lastIndexOf(n) && arr2.indexOf(n) === -1) {
        arr2.push(n);
      }
    });
    return arr2;
